...
formatOutput (haltingState, finalTape) =
    let nonBlanks = filter (/= "_") finalTape
    in "Final tape (non-blank characters): " ++ unwords nonBlanks ++ 
        ...
        (filePath:_) -> do
            ...
            case result of
                Left err -> do
                    putStrLn ("Error: " ++ err)
                    exitFailure
                Right finalTape -> putStrLn ("Final tape: " ++ finalTape)